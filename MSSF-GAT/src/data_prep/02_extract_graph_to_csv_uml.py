"""
02_extract_graph_to_csv_uml.py

功能简介：
根据 01_filter_metadata_uml.py 生成的 valid_graph_ids.csv，
从 ModelSet UML 的 graph/*.json 中提取图结构信息，生成三张原始表：

1. nodes_raw.csv
2. edges_raw.csv
3. graphs_raw.csv

本文件是 UML 专用版本，和通用/ecore 版本相比，主要特点是：

1. 只支持 --dataset uml
2. 专门适配 repo-genmymodel-uml 的 graph json 匹配方式
3. 节点名称采用 UML 专用清洗逻辑：
   - 只优先使用 name / label / qualifiedName
   - 不再使用 id 作为 node_name 兜底
   - 若 node_name 为纯数字，则统一替换为 [MISSING]
4. 输出统计中会额外打印：
   - 纯数字 node_name 数量
   - 被替换为 [MISSING] 的数量

为什么不再使用 id 兜底：
- UML 图中大量节点的 id 只是工具内部编号，不具备稳定语义
- 若后续要构造 joint text，例如：
    name: Transition; type: Class
  那么把无语义数字带入文本编码器通常只会增加噪声
- 因此这里统一将纯数字名称视为“无有效名称”

输入：
- data/uml/01_filtered_metadata/valid_graph_ids.csv
- data/raw/modelset/modelset/graph/repo-genmymodel-uml/**/*.json
...
运行方式：
python src/data_prep/02_extract_graph_to_csv_uml.py --dataset uml
"""
...
PURE_DIGIT_PATTERN = re.compile(r"^\d+$")


def is_pure_numeric_name(s: str) -> bool:
    return bool(PURE_DIGIT_PATTERN.fullmatch(s.strip()))
...
def extract_raw_node_name(node_obj: Dict[str, Any]) -> str:
    """
    UML 中优先使用真正可能承载语义的字段。
    注意：
    - 不再使用 id 作为名称兜底，因为 id 往往只是内部编号
    - qualifiedName 仅作为最后补充
    """
    for key in ["name", "label", "qualifiedName"]:
        val = node_obj.get(key)
        if val is not None:
            s = str(val).strip()
            if s:
...
def clean_uml_node_name(raw_name: str) -> Tuple[str, bool]:
    """
    返回：
    - cleaned_name
    - was_numeric_replaced: 是否因为纯数字名称而替换为 [MISSING]
    ...
    if is_pure_numeric_name(raw_name):
        return "[MISSING]", True
...
def extract_graphs_to_csv(
    ...
    stats: Dict[str, Any] = {
        ...
        "pure_numeric_name_count": 0,
        "numeric_name_cleared_count": 0,
    }
    ...
    for _, row in valid_df.iterrows():
        ...
        for local_idx, node_obj in enumerate(nodes):
            ...
            raw_node_name = extract_raw_node_name(node_obj)

            if is_pure_numeric_name(raw_node_name):
                stats["pure_numeric_name_count"] += 1

            node_name, was_numeric_replaced = clean_uml_node_name(raw_node_name)
            if was_numeric_replaced:
                stats["numeric_name_cleared_count"] += 1

            is_name_missing = 1 if node_name == "[MISSING]" else 0
...
def print_extraction_summary(
    ...
    print("-" * 80)
    print(f"纯数字 node_name 数量: {stats['pure_numeric_name_count']}")
    print(f"按 UML 规则替换为 [MISSING] 的数量: {stats['numeric_name_cleared_count']}")
    print("-" * 80)
...
# =========================================================
def main() -> None:
    parser = argparse.ArgumentParser(description="从 UML graph json 提取 nodes/edges/graphs 三张表")
    parser.add_argument("--dataset", type=str, required=True, choices=["uml"], help="仅支持 uml")
